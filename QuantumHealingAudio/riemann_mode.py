    ...
    try:
        with open(md_path, 'r', encoding='utf-8', errors='ignore') as f:
            for line in f:
                ...
                if line.startswith('|-') or 'Zero #' in line:
                    continue
                cols = [c.strip() for c in line.strip('|').split('|')]
                if len(cols) < 2 or not cols[0].isdigit():
                    continue
    ...
    try:
        with open(md_path, 'r', encoding='utf-8', errors='ignore') as f:
            for line in f:
                ...
                if len(cols) < 2 or not cols[0].isdigit():
                    continue
                # Remove trailing dots or ellipses
                t_str = cols[1].rstrip('.')
                try:
                    t_val = float(t_str)
                except ValueError:
...
def main():
    ...
    print("Imaginary parts (first 10 zeros):")
    print(zeros)
    print("Intervals between successive zeros:")
    print(intervals)

    # Suggest beat frequencies (1 / interval)
    beat_freqs = [round(1.0 / dt, 3) if dt > 0 else 0 for dt in intervals]
    print("Suggested beat frequencies (Hz):")
    print(beat_freqs)
